...
#from classif_experim.hf_wrapper.hf_skelarn_wrapper import SklearnTransformerClassif
#from hf_wrapper.hf_skelarn_wrapper import SklearnTransformerClassif



def evaluate_on_test_dataset(model: SklearnTransformerClassif, lang: str, positive_class: str): #, append_results=True, outdir='.'):
    '''
    ...
    #if append_results:
    #    result_file = f"{outdir}/evaluation.prototext"
    #    with open(result_file, 'a') as f:
    #        f.write(f"\n### Results for model: {model._hf_model_label} ###\n")
    #        with open(result_file, 'r') as original_file:
    #            f.write(original_file.read())

def build_eval_model(lang, positive_class='critical'):

    #for config in model_configs:
    #    model_name = config.get('model_name')
    #    model_label = config.get('model_label')
    #    print(f"Building and evaluating model: {model_label} ({model_name})")

    #model = load_or_build_classif_fulltrain_model(lang, model_name, model_label=model_label, positive_class=positive_class)

    hf_model_id = 'digitalepidemiologylab/covid-twitter-bert' if lang == 'en' else 'dccuchile/bert-base-spanish-wwm-cased'
    model = load_or_build_classif_fulltrain_model(lang, hf_model_id, model_label='bert-base', positive_class=positive_class)
    evaluate_on_test_dataset(model, lang, positive_class=positive_class)

if __name__ == '__main__':
    #models_to_evaluate = [
    #    {'model_name': 'bert-base-cased', 'model_label': 'bert-base-cased'},
    #    {'model_name': 'microsoft/deberta-v3-large', 'model_label': 'deberta'},
    #    {'model_name': 'roberta-large', 'model_label': 'roberta-large'},
    #    {'model_name': 'digitalepidemiologylab/covid-twitter-bert', 'model_label': 'ct-bert'}
    #]
    #build_eval_models('en', models_to_evaluate, positive_class='critical')
    build_eval_model('en')
