...
#import classif_experiment_runner
#from classif_experiment_runner import setup_logging
#from classif_utils import classif_scores
#from pynvml_helpers import print_cuda_devices
...
from seqlab_sklearn_wrapper_multitask import OppSequenceLabelerMultitask
...
##
#neptune_run = neptune.init_run(project="duygu/thesis")
##
...
def run_crossvalid_seqlab_transformers(lang, model_label, model_params, num_folds=5,
    ...
    for train_index, test_index in foldgen.split(docs, classes):
        ...
        fold_macro_f1 = scores.get('macro-F1', -float('inf'))
        if fold_macro_f1 > best_macro_f1:
            if best_model_dir and os.path.exists(best_model_dir):
                ...
                try:
                    shutil.rmtree(best_model_dir)  # Delete the old folder
            ...
            best_model_dir = os.path.join(parent_folder, f"seqlab_model_{model_label}_fold_{fold_index+1}_rseed[{rseed}]")
            logger.info(f"Best model directory path: {best_model_dir}")

            if not os.path.exists(best_model_dir):
                logger.info(f"Creating directory: {best_model_dir}")
            ...
            try:
                ...
                logger.error(f"Failed to save model to {best_model_dir}. Error: {e}")
                sys.exit(f"Critical Error: Unable to save the model at fold {fold_index + 1}")

            expected_files = ["model.pth", "config.json"]
            missing_files = [f for f in expected_files if not os.path.exists(os.path.join(best_model_dir, f))]
            if missing_files:
                logger.error(f"Model save incomplete. Missing files: {missing_files}")
                sys.exit(f"Critical Error: Model save incomplete. Missing files: {missing_files}")

            logger.info(f"New best model saved with macro-F1: {best_macro_f1:.4f} at {best_model_dir}")
        else:
            logger.info(f"No improvement in macro-F1. Current macro-F1: {fold_macro_f1:.4f}, Best macro-F1: {best_macro_f1:.4f}")

        # Terminate if fold_macro_f1 score is invalid
        if fold_macro_f1 == -float('inf'):
            logger.error(f"Invalid macro-F1 score: {fold_macro_f1}. Terminating script.")
            sys.exit(f"Critical Error: Invalid macro-F1 score detected at fold {fold_index + 1}")
    ...
    logger.info(f"Best model saved to: {best_model_dir} with macro-F1: {best_macro_f1:.4f}")
...
def build_seqlab_model(model_label, rseed, model_params, task_labels, task_indices):
    ...
    model = OppSequenceLabelerMultitask(hf_model_label=model_label, rnd_seed=rseed, task_labels=task_labels, task_indices=task_indices, **model_params)
    if hasattr(model, 'model') and isinstance(model.model, MultiTaskModel):
        logger.info("Binding MultiTaskModel's save_model to OppSequenceLabelerMultitask.")
        model.save_model = model.model.save_model
    #else:
    #    def save_model_fallback(save_dir):
    ...
    #    model.save_model = save_model_fallback
