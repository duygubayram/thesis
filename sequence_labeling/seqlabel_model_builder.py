...
def build_seqlab_model_on_full_train(lang, hf_model_name, model_label, rseed=35412, save=False):
    ...
    while try_batch_size >= 1:
        try:
            ...
            model.fit_(docs)
            if save:
                # Use MultiTaskModel save_model if applicable
                if hasattr(model, 'model') and isinstance(model.model, MultiTaskModel):
                    model.model.save_model(mfolder)
                    logging.info(f"Model saved successfully to {mfolder}.")
                else:
                    # Default save for SklearnTransformerBase
                    model.save(mfolder)
                    logging.info(f"Model saved successfully to {mfolder}.")
            return model
